<?php 
$pageTitle = "Loïc Parent • Ds Wallonie";
include( 'header.php' );
?>

<h1 class="projectTitle-1 title2">Ds Wallonie</h1>
<div id="content" class="content">
	<section class="infoProject">
		<div class="centerZone">
			<h2 class="projectTitle title2 hidden">Informations sur le projet</h2>
			<div class="left">
				<div class="pictureZone">
					<img src="./img/dswallonie.png" alt="Ds Wallonie">
				</div>
				<a href="./dswallonie/index.html" class="infoProject__link">Accéder au site</a>
			</div>
			<div class="right">
				<p>En deuxième année de mon cursus à la Haute &Eacute;cole de la Province de Liège, j’ai participé à un coucours proposé par l’agence Ds Wallonie située à Liège (partenaire de Xérox).</p>
				<p>Pour ce concours, nous devions proposer une refonte complète de leur site internet et donc leurs proposer un design remis aux goût du jour.</p>
				<p>Je n’ai pas remporté le concours mais j’ai néanmoins passer la première qualification et terminé dans les trois finalistes.</p>
				<p>Trêve d’explications, je vous propose de découvrir le projet.</p>
			</div>
			<div class="clear"></div>
		</div>
	</section>
</div>

<?php include( 'footer.php' ); ?>